...
def setup_students():
    ...
    students.extend([camila, ximena, peña, tomas, emma, susu, aitana, carlosA,
                     carlosJ, roger, maria, chuy, oscar, jonathan])
...
def draw_combined_graph():
    ...
    for student in students:
        G.add_node(student.name, type='student', career=student.career, hobby=student.hobbies)
        G.add_edge(student.name, student.building, type='assigned')
        for friend in student.friends:
            if not G.has_edge(student.name, friend.name):
    ...
    student_nodes = [n for n, attr in G.nodes(data=True) if attr['type'] == 'student']
    student_colors = [career_colors.get(G.nodes[n]['career'], 'gray') for n in student_nodes]

    nx.draw_networkx_nodes(G, pos, nodelist=building_nodes, node_color='lightgreen', node_size=500)
    nx.draw_networkx_nodes(G, pos, nodelist=student_nodes, node_color=student_colors, node_size=500)
    ...
    assigned_edges = [(u, v) for u, v, d in G.edges(data=True) if d['type'] == 'assigned']

    nx.draw_networkx_edges(G, pos, edgelist=building_edges, edge_color='green', style='solid')
    nx.draw_networkx_edges(G, pos, edgelist=friendship_edges, edge_color='blue', style='dashed')
    nx.draw_networkx_edges(G, pos, edgelist=assigned_edges, edge_color='gray', style='dotted')

    labels = {}
    for node in G.nodes():
        if G.nodes[node]['type'] == 'student':
            hobby = G.nodes[node]['hobby']
            labels[node] = f"{node}\n({hobby})"
        else:
            labels[node] = node
    ...
    plt.title("WichoTinder - Estudiantes por carrera y hobby")
    plt.axis('off')
    plt.show()
